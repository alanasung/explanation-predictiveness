...
|---|---|---|
| Config schema | `simulate.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `simulate.configs.loader` | Hydra compose API |
| Device | `simulate.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `simulate.models.registry` | Pinned revisions + hardware fit |
| Hooks | `simulate.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `simulate.models.generation` | Chat template path recording |
| Data | `simulate.data` | Synthetic items, splits, manifests |
| Evaluation | `simulate.evaluation` | Metrics, failure harness, protocols |
| Ablation | `simulate.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `simulate.reporting` | Aggregate → MD/TeX/figures |
| Cache | `simulate.cache` | Versioned artifact store |
...
`src/simulate/simulate/` are preserved across regeneration.
