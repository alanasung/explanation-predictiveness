...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `simulate.configs` | resolved `config.yaml` |
| Build data | `simulate.data` | splits + manifest |
| Load model | `simulate.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `simulate.reporting` | `results.json` + tables + figures |
...
├── src/simulate/       # installable library (print-free)
...
Ablations live under `src/simulate/ablation/` and return structured dicts. Presets
